...
if __name__ == '__main__':
    for reps in range(10):
        ...
        signals = generate_spike_times(10, 90, 3, 7)
        sg2_times = np.random.choice(signals, size=int(len(signals)/3), replace=False)
        sg2_src = sim.Population(1, sim.SpikeSourceArray(spike_times=sg2_times))
        sg1_times = [i for i in signals if i not in sg2_times]
